  ...
  useEffect(() => {
    ...
    const initApp = async () => {
      try {
        ...
        // 4. THE Meat (Stylized Game Icon)
        // Using a clear, high-res stylized cartoon Meat
        const meatTexture = await PIXI.Assets.load('https://cdn-icons-png.flaticon.com/512/590/590800.png');
        
        if (destroyed) return;

            const meat = new PIXI.Sprite(meatTexture);
            meat.anchor.set(0.5); // Center the anchor point
            meat.scale.set(0.36); // Adjust scale for better fit
            meat.y = -6; // Slightly adjust vertical position

        // Subtle drop shadow for the meat itself
        const meatShadow = new PIXI.Graphics()
          .ellipse(0, 45, 30, 8)
          .fill({ color: 0x000000, alpha: 0.2 });
        
        root.addChild(meatShadow);
        root.addChild(meat);
        ...
        //   root.x = 175 + Math.cos(elapsed * 0.3) * 8;
          ...
          // Hover for the Meat
          meat.y = -10 + Math.cos(elapsed * 0.8) * 4; //up down
          meat.rotation = Math.sin(elapsed * 0.2) * 0.05; //tilt
          meatShadow.scale.set(1 + Math.cos(elapsed * 0.8) * 0.1); //shadow size
          meatShadow.alpha = 0.2 - Math.abs(Math.cos(elapsed * 0.8)) * 0.05;
          // Badge pulse

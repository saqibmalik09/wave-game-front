  ...
  useEffect(() => {
    ...
    const initApp = async () => {
      try {
        ...
        // 4. THE Banana (Stylized Game Icon)
        // Using a clear, high-res stylized cartoon Banana
        const bananaTexture = await PIXI.Assets.load('https://cdn-icons-png.flaticon.com/512/590/590764.png');
        
        if (destroyed) return;

            const banana = new PIXI.Sprite(bananaTexture);
            banana.anchor.set(0.5); // Center the anchor point
            banana.scale.set(0.34); // Adjust scale for better fit
            banana.y = -6; // Slightly adjust vertical position

        // Subtle drop shadow for the banana itself
        const bananaShadow = new PIXI.Graphics()
          .ellipse(0, 45, 30, 8)
          .fill({ color: 0x000000, alpha: 0.2 });
        
        root.addChild(bananaShadow);
        ...
        //   root.x = 175 + Math.cos(elapsed * 0.3) * 8;
          ...
          banana.y = -10 + Math.cos(elapsed * 0.8) * 4; //up down
          banana.rotation = Math.sin(elapsed * 0.2) * 0.05; //tilt
          bananaShadow.scale.set(1 + Math.cos(elapsed * 0.8) * 0.1); //shadow size
          bananaShadow.alpha = 0.2 - Math.abs(Math.cos(elapsed * 0.8)) * 0.05;

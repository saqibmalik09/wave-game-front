  ...
  useEffect(() => {
    ...
    const initApp = async () => {
      try {
        ...
        // 4. THE Cherry (Stylized Game Icon)
        // Using a clear, high-res stylized cartoon Cherry
        const CherryTexture = await PIXI.Assets.load('https://cdn-icons-png.flaticon.com/512/590/590774.png');
        
        if (destroyed) return;

            const Cherry = new PIXI.Sprite(CherryTexture);
            Cherry.anchor.set(0.5); // Center the anchor point
            Cherry.scale.set(0.34); // Adjust scale for better fit
            Cherry.y = -6; // Slightly adjust vertical position

        // Subtle drop shadow for the Cherry itself
        const CherryShadow = new PIXI.Graphics()
          .ellipse(0, 45, 30, 8)
          .fill({ color: 0x000000, alpha: 0.2 });
        
        root.addChild(CherryShadow);
        ...
        //   root.x = 175 + Math.cos(elapsed * 0.3) * 8;
          ...
          // Hover for the Cherry
          Cherry.y = -10 + Math.cos(elapsed * 0.8) * 4; //up down
          Cherry.rotation = Math.sin(elapsed * 0.2) * 0.05; //tilt
          CherryShadow.scale.set(1 + Math.cos(elapsed * 0.8) * 0.1); //shadow size
          CherryShadow.alpha = 0.2 - Math.abs(Math.cos(elapsed * 0.8)) * 0.05;

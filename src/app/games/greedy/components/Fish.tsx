  ...
  useEffect(() => {
    ...
    const initApp = async () => {
      try {
        ...
        // 4. THE Fish (Stylized Game Icon)
        // Using a clear, high-res stylized cartoon Fish
        const fishTexture = await PIXI.Assets.load('https://cdn-icons-png.flaticon.com/512/782/782930.png');
        
        if (destroyed) return;

            const fish = new PIXI.Sprite(fishTexture);
            fish.anchor.set(0.5); // Center the anchor point
            fish.scale.set(0.34); // Adjust scale for better fit
            fish.y = -6; // Slightly adjust vertical position

        // Subtle drop shadow for the fish itself
        const fishShadow = new PIXI.Graphics()
          .ellipse(0, 45, 30, 8)
          .fill({ color: 0x000000, alpha: 0.2 });
        
        root.addChild(fishShadow);
        root.addChild(fish);
        ...
        //   root.x = 175 + Math.cos(elapsed * 0.3) * 8;
          ...
          // Hover for the fish
          fish.y = -10 + Math.cos(elapsed * 0.8) * 4; //up down
          fish.rotation = Math.sin(elapsed * 0.2) * 0.05; //tilt
          fishShadow.scale.set(1 + Math.cos(elapsed * 0.8) * 0.1); //shadow size
          fishShadow.alpha = 0.2 - Math.abs(Math.cos(elapsed * 0.8)) * 0.05;

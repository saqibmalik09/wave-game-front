  ...
  useEffect(() => {
    ...
    const initApp = async () => {
      try {
        ...
        // 4. THE Burger (Stylized Game Icon)
        // Using a clear, high-res stylized cartoon Burger
        const burgerTexture = await PIXI.Assets.load('https://cdn-icons-png.flaticon.com/512/590/590803.png');
        
        if (destroyed) return;

            const burger = new PIXI.Sprite(burgerTexture);
            burger.anchor.set(0.5); // Center the anchor point
            burger.scale.set(0.34); // Adjust scale for better fit
            burger.y = -6; // Slightly adjust vertical position

        // Subtle drop shadow for the burger itself
        const burgerShadow = new PIXI.Graphics()
          .ellipse(0, 45, 30, 8)
          .fill({ color: 0x000000, alpha: 0.2 });
        
        root.addChild(burgerShadow);
        root.addChild(burger);
        ...
        //   root.x = 175 + Math.cos(elapsed * 0.3) * 8;
          ...
          // Hover for the burger
          burger.y = -10 + Math.cos(elapsed * 0.8) * 4; //up down
          burger.rotation = Math.sin(elapsed * 0.2) * 0.05; //tilt
          burgerShadow.scale.set(1 + Math.cos(elapsed * 0.8) * 0.1); //shadow size
          burgerShadow.alpha = 0.2 - Math.abs(Math.cos(elapsed * 0.8)) * 0.05;

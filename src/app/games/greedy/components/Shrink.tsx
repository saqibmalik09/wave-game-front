  ...
  useEffect(() => {
    ...
    const initApp = async () => {
      try {
        ...
        // 4. THE Shrink (Stylized Game Icon)
        // Using a clear, high-res stylized cartoon Shrink
        const shrinkTexture = await PIXI.Assets.load('https://cdn-icons-png.flaticon.com/512/782/782934.png');
        
        if (destroyed) return;

            const shrink = new PIXI.Sprite(shrinkTexture);
            shrink.anchor.set(0.5); // Center the anchor point
            shrink.scale.set(0.34); // Adjust scale for better fit
            shrink.y = -6; // Slightly adjust vertical position

        // Subtle drop shadow for the shrink itself
        const shrinkShadow = new PIXI.Graphics()
          .ellipse(0, 45, 30, 8)
          .fill({ color: 0x000000, alpha: 0.2 });
        
        root.addChild(shrinkShadow);
        root.addChild(shrink);
        ...
        //   root.x = 175 + Math.cos(elapsed * 0.3) * 8;
          ...
          // Hover for the Shrink
          shrink.y = -10 + Math.cos(elapsed * 0.8) * 4; //up down
          shrink.rotation = Math.sin(elapsed * 0.2) * 0.05; //tilt
          shrinkShadow.scale.set(1 + Math.cos(elapsed * 0.8) * 0.1); //shadow size
          shrinkShadow.alpha = 0.2 - Math.abs(Math.cos(elapsed * 0.8)) * 0.05;

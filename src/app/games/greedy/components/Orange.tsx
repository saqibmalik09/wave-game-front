  ...
  useEffect(() => {
    ...
    const initApp = async () => {
      try {
        ...
        // 4. THE Orange (Stylized Game Icon)
        // Using a clear, high-res stylized cartoon Orange
        const OrangeTexture = await PIXI.Assets.load('https://cdn-icons-png.flaticon.com/512/590/590767.png');
        
        if (destroyed) return;

            const Orange = new PIXI.Sprite(OrangeTexture);
            Orange.anchor.set(0.5); // Center the anchor point
            Orange.scale.set(0.34); // Adjust scale for better fit
            Orange.y = -6; // Slightly adjust vertical position

        // Subtle drop shadow for the Orange itself
        const OrangeShadow = new PIXI.Graphics()
          .ellipse(0, 45, 30, 8)
          .fill({ color: 0x000000, alpha: 0.2 });
        
        root.addChild(OrangeShadow);
        ...
        //   root.x = 175 + Math.cos(elapsed * 0.3) * 8;
          ...
          // Hover for the Orange
          Orange.y = -10 + Math.cos(elapsed * 0.8) * 4; //up down
          Orange.rotation = Math.sin(elapsed * 0.2) * 0.05; //tilt
          OrangeShadow.scale.set(1 + Math.cos(elapsed * 0.8) * 0.1); //shadow size
          OrangeShadow.alpha = 0.2 - Math.abs(Math.cos(elapsed * 0.8)) * 0.05;
          // Badge pulse
